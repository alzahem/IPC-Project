...
void cleanUp(const int& shmid, const int& msqid, void* sharedMemPtr)
{
	...
	shmdt(sharedMemPtr);
	std::cout << "Dettached from shared memory successfully.\n\n";
...
void send(const char* fileName)
{
	...
	while(!feof(fp))
	{
		...
		if(msgrcv(msqid, &rcvMsg, 0, RECV_DONE_TYPE, 0)) {
			...
			std::cout << "Message of type RECV_DONE_TYPE recieved. \n";
		}
	...
	msgsnd(msqid, &sndMsg, sizeof(sndMsg), 0) == -1;
	std::cout << "Empty message sent successfully. \n";
